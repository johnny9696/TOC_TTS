        ...
        for batchs in loader:
            ...
            for batch in batchs:
                ...
                if step % grad_acc_step == 0:
                    # Backward
                    total_loss = total_loss /float(grad_acc_step)
                    total_loss.backward()
                ...
                if step % log_step == 0:
                    ...
                    train_logger.add_scalar('grad_norm',grad_norm,step)

                if step % synth_step == 0:
                    with torch.no_grad():
                        output1 = model(*(batch), gen=True, steps=step)
                    fig, fig1, wav_reconstruction, wav_prediction= synth_one_sample(
                        batch,
                        output1,
                        vocoder,
                        model_config,
                    ...
                    log(
                        train_logger,
                        fig=fig,
                        tag="Training/Mel_Spec",
                        step = step
                    )
                    log(
                        train_logger,
                        fig=fig1,
                        tag="Training/Condition",
                        step = step
                    )
