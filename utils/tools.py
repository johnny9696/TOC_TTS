...
def to_device(data, device):
    if len(data) == 21:
        (text, t_length, max_text_length, bert, b_length,
         ...
         t_duration, token_dur, pitch, energy, cutmel, cutw2v) = data

        text = text.to(device)
        t_length = t_length.to(device)
        max_text_length = max_text_length.to(device)
        bert = bert.to(device)
        b_length = b_length.to(device)
        max_bert_length = max_bert_length.to(device)
        sbert = sbert.to(device)
        h_bert = h_bert.to(device)
        ...
        p_duration = p_duration.to(device)
        t_duration = t_duration.to(device)
        token_dur = token_dur.to(device)
        pitch = pitch.to(device)
        energy = energy.to(device)
        cutmel = cutmel.to(device)
        cutw2v = cutw2v.to(device)
...
def synth_one_sample(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    mel_target = targets[11][0, :mel_len].detach().transpose(0, 1)
    mel_prediction = predictions[1][0, :, :mel_len].detach()
    duration = targets[14][0, :src_len].detach().cpu().numpy()
    try :
        mel_condition = predictions[12][0, :src_len].detach().transpose(0, 1)
        bert_mu = predictions[13][0, 0, :src_len].detach().transpose(0, 1)
        bert_logs = predictions[13][1, 0, :src_len].detach().transpose(0, 1)
        phoneme = predictions[14][0, :src_len].detach().transpose(0, 1)
    except :
        pass

    if preprocess_config["preprocessing"]["pitch"]["feature"] == "phoneme_level":
        pitch = targets[17][0, :src_len].detach().cpu().numpy()
        pitch = expand(pitch, duration)
    else:
        pitch = targets[17][0, :mel_len].detach().cpu().numpy()
    if preprocess_config["preprocessing"]["energy"]["feature"] == "phoneme_level":
        energy = targets[18][0, :src_len].detach().cpu().numpy()
        energy = expand(energy, duration)
    else:
        energy = targets[18][0, :mel_len].detach().cpu().numpy()
    ...
    return fig, fig1, wav_reconstruction, wav_prediction
...
def plot_condition(data, titles):
    ...
    for i in range(len(data)):
        condition = data[i]
        axes[0][i].imshow(condition, origin="lower")
        axes[0][i].set_aspect(2.5, adjustable="box")
        axes[0][i].set_ylim(0, condition.shape[0])
